...
.controller('CoachHomeCtrl',
  ['$scope','$state','$stateParams','userINFO','Storage','Users','CONFIG','$timeout','QRScan',
  function($scope,$state,$stateParams,userINFO,Storage,Users,CONFIG,$timeout,QRScan) { //LRZ
...
//lrz20151125
.controller('CoachScheduleDtlCtrl',['$scope','$state','$stateParams','$ionicHistory','ScheduleService','$ionicModal','$timeout','PageFunc',function($scope,$state,$stateParams,$ionicHistory,ScheduleService,$ionicModal,$timeout,PageFunc){
...
.controller('myPatientCtrl', ['$rootScope', '$ionicScrollDelegate', '$ionicPopover','$filter','$ionicModal', '$ionicPopup','$ionicLoading','$scope', '$state','$timeout','$interval','$ionicHistory','Storage' ,'userINFO','PageFunc','CONFIG','QRScan' ,function($rootScope,$ionicScrollDelegate,$ionicPopover,$filter,$ionicModal, $ionicPopup,$ionicLoading,$scope, $state,$timeout,$interval,$ionicHistory,Storage,userINFO,PageFunc,CONFIG,QRScan){
  ...
  var PatientsList=new Array();//输出到页面的json数组
  ...
  var DOinitial = function(){
    ...
    categories=[
      [{Name:'高血压',checked:false},{Name:'糖尿病',checked:false},{Name:'心衰',checked:false}],
    ...
    ranks=[
      {Name:'依从率最低',ordername:'SMSCount desc,ComplianceRate,Status desc,RemainingDays,StartDate desc',clicked:false},
...
.controller('newpatientsCtrl', ['$ionicPopover', '$scope', '$state','$ionicLoading','$interval','$timeout' ,'userINFO','PageFunc','Storage','CONFIG','QRScan' ,function($ionicPopover,$scope, $state,$ionicLoading, $interval,$timeout,userINFO,PageFunc,Storage,CONFIG,QRScan){
...
.controller('newbasicinfoCtrl',['$scope','$state','Storage','Users','Dict','$ionicPopup','$timeout','$ionicScrollDelegate','$ionicLoading','userservice','GetBasicInfo','BasicDtlInfo','PageFunc',function($scope,$state,Storage,Users,Dict,$ionicPopup,$timeout,$ionicScrollDelegate,$ionicLoading,userservice,GetBasicInfo,BasicDtlInfo,PageFunc){
                    ...
                    if($scope.InsuranceTypes!=""){
                      ...
                      GetBasicInfo.GetBasicInfoByPid($scope.users.UserId).then(
                        function(data){
                          ...
                          $scope.users.InsuranceType=data.InsuranceTypeText;
  ...
  $scope.users.Birthday=Storage.get('b');
    ...
    if($scope.PhoneNumber.Value && !(phoneReg.test($scope.PhoneNumber.Value) || isPhone.test($scope.PhoneNumber.Value))){
      $scope.logStatus='请输入正确的联系电话';
      hide();
      return;
    }
    if($scope.EmergencyContactPhoneNumber.Value && !(phoneReg.test($scope.EmergencyContactPhoneNumber.Value) || isPhone.test($scope.EmergencyContactPhoneNumber.Value))){
...
//GL 20151101 创建计划
.controller('CreateCtrl', ['$scope','$rootScope', '$http', '$state', '$stateParams', 'PlanInfo', 'Dict', '$ionicPopup', 'Storage', '$ionicHistory', '$ionicLoading', function($scope,$rootScope, $http, $state, $stateParams, PlanInfo, Dict, $ionicPopup, Storage, $ionicHistory, $ionicLoading){
    $scope.create = {};
...
.controller('TaskListCtrl', ['$scope', '$http', '$state', '$stateParams', 'PlanInfo', '$ionicPopup', '$ionicHistory','Storage', '$ionicLoading', "$rootScope",function($scope, $http, $state, $stateParams, PlanInfo, $ionicPopup, $ionicHistory, Storage, $ionicLoading,$rootScope){
        ...
        promise.then(function(data) {
            for (var i = 0; i < data.length; i++)
            {
                ...
                if (data[i].InvalidFlag === "1")
                {
                    ...
                    if($rootScope.firstLoad['taskList'])
                      $rootScope.showCheckIcon[data[i].Type]=1;
                }
                else
                {
            ...
            $rootScope.firstLoad['taskList']=false;
            GetSBP();
        }, function(data) {
        });
    }
    ...
    $scope.onClickSubmit = function(){
        ...
        // $ionicHistory.goBack();
        var POSTAdd=[],POSTDel=[];
        for(var Typekey in $rootScope.TempList.AddList){
          POSTAdd=POSTAdd.concat($rootScope.TempList.AddList[Typekey]);
        }
        for(var Typekey in $rootScope.TempList.DeleteList){
          POSTDel=POSTDel.concat($rootScope.TempList.DeleteList[Typekey]);
        }
        var POSTForDel = function(dataArr){
          if(dataArr.length){
            PlanInfo.DeleteTask(dataArr)
            .then(function(data){
...
.controller('MainPlanCtrl',['$scope', '$http', '$state', '$stateParams', 'PlanInfo', '$ionicPopup', '$ionicHistory', 'Storage', '$ionicLoading', "Dict", "$rootScope",function($scope, $http, $state, $stateParams, PlanInfo, $ionicPopup, $ionicHistory, Storage, $ionicLoading, Dict,$rootScope){
    ...
    function GettaskList()
    {
        ...
        promise.then(function(data) {
          if($rootScope.firstLoad[Type]){
            for(var i=0; i <data.length; i++){
              data[i].ControlType = data[i].InvalidFlag === "1"?true:false;
                arry.push(data[i].ControlType);
                if (Type == "TB"){
                  data[i].Instruction =parseInt(data[i].Instruction);
                }
            }
          }else{
            for(var i=0; i <data.length; i++){
              arry.push(data[i].InvalidFlag === "1"?true:false);
              data[i].ControlType = $rootScope.TempList.flag[Type][i];
              if(data[i].ControlType){
                for(var k in $rootScope.TempList.AddList[Type]){
                  if($rootScope.TempList.AddList[Type][k].Code==data[i].Code)
                    data[i].Instruction=$rootScope.TempList.AddList[Type][k].Instruction;
                }
                if (Type == "TB"){
                  data[i].Instruction =parseInt(data[i].Instruction);
                }
  ...
  //按下确定键触发 LRZ 20160108 将所有列表暂时存到rootscope
    $scope.Confirm = function()
    {
        $rootScope.firstLoad[Type]=false;
        $rootScope.TempList.flag[Type]=[];
        for(var i in $scope.task.list)
          $rootScope.TempList.flag[Type].push($scope.task.list[i].ControlType);
        var AddList = new Array();
        var DeleteList = new Array();
        var FlagBefore = false;
        var FlagNow = false;
        ...
        $rootScope.showCheckIcon[Type]=AddList.length;
        if ((!FlagBefore) && (FlagNow)) //插入上级条目
        {
            AddList.push({"PlanNo":PlanNo,
                         "Type":Type,
        ...
        // if(AddList.length > 0){
            ...
            // for (var i = AddList.length - 1; i >= 0; i--) {
             $rootScope.TempList.AddList[Type]=AddList;
            // };
            // if (DeleteList.length > 0){
                // $rootScope.TempList.DeleteList.concat(DeleteList);
                  // for (var i = DeleteList.length - 1; i >= 0; i--) {
                   $rootScope.TempList.DeleteList[Type]=DeleteList;
        //           };
        //           if (localStorage.getItem("isManage") == "Yes")
        //             {
        //               window.location.href = "#/manage/taskList";
...
.controller('healthEducationCtrl', ['$scope', '$http', '$state', '$stateParams', 'PlanInfo', '$ionicHistory', 'Storage', '$ionicLoading', '$ionicPopup', "$rootScope",function($scope, $http, $state, $stateParams, PlanInfo, $ionicHistory, Storage, $ionicLoading, $ionicPopup,$rootScope){
    ...
    function GettaskList()
    {
        promise = PlanInfo.GetTasks(PlanNo, Type + "0000")
        .then(function(data) {
          if($rootScope.firstLoad[Type]){
            for(var i=0; i <data.length; i++){
              data[i].ControlType = data[i].InvalidFlag === "1"?true:false;
    ...
    $scope.task.AddList = {};
    $scope.task.DeleteList = {};
    if(!$rootScope.firstLoad[Type])
      $scope.task.detailList=$rootScope.TempList.eduDetail;
    ...
    function ShowPop(index)
    {
        ...
        var ArryOrl = $scope.task.detailList[index].OriginFlag;
        var Arry2=[];
        for(var i =0;i<$scope.task.secondlist.length;++i){
          Arry2.push($scope.task.secondlist[i].ControlType);
        }
        ...
        myPopup.then(function(res) {
            if (res)
            {
                $scope.task.AddList[piType]=[];
                $scope.task.DeleteList[piType]=[];
                var FlagBefore = false;
                var FlagNow = false;
                for (var i=0; i < ArryOrl.length; i++)
                {
                    if (ArryOrl[i])
                    {
                        FlagBefore = true;
                ...
                for (var i=0; i < $scope.task.secondlist.length; i++)
                {
                    if (($scope.task.secondlist[i].ControlType) && (!ArryOrl[i])) //插入数据
                    {
                        $scope.task.AddList[piType].push({"PlanNo":PlanNo,
                    ...
                    if((!$scope.task.secondlist[i].ControlType) && (ArryOrl[i])) //删除数据
                    {
                        $scope.task.DeleteList[piType].push({"PlanNo":PlanNo,
                                         "Type":$scope.task.secondlist[i].Type,
                                         "Code":$scope.task.secondlist[i].Code,
                                         "SortNo":'1'});
                    }
    ...
    $scope.Confirm = function()
    {
        ...
        var cout=0;
        for(var i=0;i<$scope.task.list.length;++i)
          cout+=$scope.task.list[i].ControlType;
        $rootScope.showCheckIcon[Type]=cout;
        for(var typeKey in $scope.task.AddList){
          for(var i in $scope.task.AddList[typeKey])
            $rootScope.TempList.AddList[Type].push($scope.task.AddList[typeKey][i]);
        }
        for(var typeKey in $scope.task.DeleteList){
          for(var i in $scope.task.DeleteList[typeKey])
            $rootScope.TempList.DeleteList[Type].push($scope.task.DeleteList[typeKey][i]);
        }
        if ((!FlagBefore) && (FlagNow)) //插入上级条目
        {
            $rootScope.TempList.AddList[Type].push({"PlanNo":PlanNo,
                         "Type":Type,
                         "Code":Type + "0000",
                         "SortNo":'1',
                         "Instruction":"",
        ...
        if ((FlagBefore) && (!FlagNow)) //删除上级条目
        {
            $rootScope.TempList.DeleteList[Type].push({"PlanNo":PlanNo,
                             "Type":Type,
                             "Code":Type + "0000",
                             "SortNo":'1'});
        }
        $rootScope.TempList.eduDetail=$scope.task.detailList;
        $rootScope.firstLoad[Type]=false;
        $rootScope.TempList.flag[Type]=[];
        for(var i in $scope.task.list)
          $rootScope.TempList.flag[Type].push($scope.task.list[i].ControlType);
...
.controller('DrugCtrl',['$scope', '$http', '$state', '$stateParams', '$ionicPopup', 'Users', 'PlanInfo', '$ionicHistory', '$ionicLoading', '$rootScope',function($scope, $http, $state, $stateParams, $ionicPopup, Users, PlanInfo, $ionicHistory, $ionicLoading,$rootScope){
    ...
    //获取任务列表
    function renderList(arr,i){
      arr[i].Description = arr[i].Instruction.split(':')[0];
      arr[i].OptionCategory = "";
      var arry = arr[i].Instruction.split(':')[1].split('|');
      for (var j = 0; j < arry.length; j++)
      {
          if (arry[j] != "")
          {
              arr[i].OptionCategory += TypeList[j] + arry[j];
          }
      }
      if (arr[i].OptionCategory != "")
      {
          arr[i].OptionCategory = arr[i].OptionCategory.substring(1);
      }
    }
    function GettaskList()
    {
        if(!$rootScope.firstLoad[Type]){
          var arrAdd = $rootScope.TempList.AddList[Type];
          for(var i in arrAdd){
            if(arrAdd[i].Code!= Type+'0000'){
              renderList(arrAdd,i);
            }else{
              var tmp=i;
            }
          ...
          arrAdd.splice(tmp,1);
          $scope.task.list = arrAdd;
          $ionicLoading.hide();
          $scope.isloaded = true;
          arrAdd=null;
          return;
        }
        PlanInfo.GetTasks(PlanNo, Type + "0000")
        .then(function(data) {
            for (var i=0; i<data.length; i++)
            {
    ...
    $scope.Confirm = function ()
    {
        ...
        //deal with 父级条目
        var len=$rootScope.TempList.DeleteList[Type].length;
        if(len && $rootScope.TempList.DeleteList[Type][len-1].Code==Type + '0000'){
          //找到，是否删除
          if($scope.task.list.length == 0){
            $rootScope.TempList.DeleteList[Type].pop();
          }
        }else{
          //没找到，是否添加
          if(len && $scope.task.list.length == 0){
            $rootScope.TempList.DeleteList[Type].push({"PlanNo":PlanNo, "Type":Type, "Code":Type + "0000", "SortNo":'1'});
          }
        }
        // if ($scope.task.DeleteList.length > 0 && $scope.task.list.length == 0)
        // {
            // if($scope.task.list.length == 0)
            // {
        ...
        $rootScope.TempList.AddList[Type]=AddList;
        if($rootScope.firstLoad[Type]){
          $rootScope.TempList.DeleteList[Type]=$scope.task.DeleteList;
          if($scope.task.DeleteList.length > 0 && $scope.task.list.length == 0)
            $rootScope.TempList.DeleteList[Type].push({"PlanNo":PlanNo, "Type":Type, "Code":Type + "0000", "SortNo":'1'});
        }
        $rootScope.firstLoad[Type]=false;
        $rootScope.showCheckIcon[Type]=$scope.task.list.length;
        $ionicHistory.goBack()
    }
